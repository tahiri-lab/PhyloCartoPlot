    ...
    def plot(self, figsize=(35, 12)):
        ...
        if self.legend_config['show_legend']:
            ...
            self.ax2.legend(
                ...
                bbox_to_anchor=self.legend_config['bbox_to_anchor'],  # ← Use from config
            ...
            if self.verbose:
                print(f"  ✓ Categorical legend added")

        # Adjust spacing based on whether we have raster or map
        if self.raster_data is not None:
            # Raster mode: space for colorbar on right
            self.fig.subplots_adjust(wspace=0.2, bottom=0.1, left=0.15)
        else:
            # Map mode: no colorbar on right, space for legend at bottom
            # Increased bottom to accommodate legend
            self.fig.subplots_adjust(wspace=0.05, bottom=0.15, left=0.1)
